...
it('최소 가격 또는 최대 가격을 수정하면 setMinPrice과 setMaxPrice 액션이 호출된다.', async () => {
  ...
  await user.type(minPriceTextInput, '1000');

  expect(setMinPriceFn).toHaveBeenCalledWith('1000');

  const maxPriceTextInput = screen.getByPlaceholderText('최대 금액');
  await user.type(maxPriceTextInput, '2000');

  expect(setMaxPriceFn).toHaveBeenCalledWith('2000');
});
